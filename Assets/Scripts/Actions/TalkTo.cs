...
public class TalkTo : Action
{
    ...
    {
        if (TalkToItem(controller, controller.player.currentLocation.itemList, noun))
            return;

        controller.currentText.text = "There is no " + noun + " here!";
    }

    private bool TalkToItem(GameController controller, List<Item>itemList, string noun)
    {
        foreach (Item item in itemList)
        {
            if (item.itemName == noun && item.itemEnabled)
            {
                ...
                controller.currentText.text = "The  " + noun + " says nothing...Lifting an eyebrow";
                return true;
            }
